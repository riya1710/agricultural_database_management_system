...
def addfarming():
    if request.method=="POST":
        ...
        db.session.add(dep)
        db.session.commit()
        flash("Farming product added","success")
...
def edit(rid):
    ...
    if request.method=="POST":
        ...
        # query=db.engine.execute(f"UPDATE `register` SET `farmername`='{farmername}',`aadharnumber`='{aadharnumber}',`age`='{age}',`gender`='{gender}',`phonenumber`='{phonenumber}',`address`='{address}',`farming`='{farmingtype}'")
        post=Register.query.filter_by(rid=rid).first()
...
def register():
    ...
    if request.method=="POST":
        ...
        db.session.commit()
        # query=db.engine.execute(f"INSERT INTO `register` (`farmername`,`aadharnumber`,`age`,`gender`,`phonenumber`,`address`,`farming`) VALUES ('{farmername}','{adharnumber}','{age}','{gender}','{phonenumber}','{address}','{farmingtype}')")
